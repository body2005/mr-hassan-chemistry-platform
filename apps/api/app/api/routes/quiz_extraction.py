...
class GeneratedQuestion(BaseModel):
    ...
    needs_points_assignment: bool = False
    correct_answer: str | None = None
    needs_review: bool = False
    needs_answer_review: bool = False
    answer_confidence: Literal["confirmed", "unknown"] = "unknown"
    explanation: str = ""
    ...
    classification_confidence: Any | None = None
...
async def extract_quiz_from_file(
    ...
    try:
        ...
        # Validate magic bytes against claimed extension to reject corrupt or forged files early
        with open(staging_file, "rb") as f_head:
            magic_header = f_head.read(16)

        if extension == ".pdf" and not magic_header.startswith(b"%PDF-"):
            raise HTTPException(
                status_code=status.HTTP_422_UNPROCESSABLE_ENTITY,
                detail={
                    "code": "CORRUPT_OR_INVALID_FILE",
                    "message": "الملف المرفوع تالف أو لا يتطابق مع صيغة PDF (توقيع الملف غير صالح).",
                },
            )
        elif extension == ".docx" and not magic_header.startswith(b"PK\x03\x04"):
            raise HTTPException(
                status_code=status.HTTP_422_UNPROCESSABLE_ENTITY,
                detail={
                    "code": "CORRUPT_OR_INVALID_FILE",
                    "message": "الملف المرفوع تالف أو لا يتطابق مع صيغة DOCX (توقيع الملف غير صالح).",
                },
            )
        elif extension == ".png" and not magic_header.startswith(b"\x89PNG\r\n\x1a\n"):
            raise HTTPException(
                status_code=status.HTTP_422_UNPROCESSABLE_ENTITY,
                detail={
                    "code": "CORRUPT_OR_INVALID_FILE",
                    "message": "الملف المرفوع تالف أو لا يتطابق مع صيغة PNG (توقيع الملف غير صالح).",
                },
            )
        elif extension in {".jpg", ".jpeg"} and not magic_header.startswith(b"\xff\xd8\xff"):
            raise HTTPException(
                status_code=status.HTTP_422_UNPROCESSABLE_ENTITY,
                detail={
                    "code": "CORRUPT_OR_INVALID_FILE",
                    "message": "الملف المرفوع تالف أو لا يتطابق مع صيغة JPEG (توقيع الملف غير صالح).",
                },
            )
        ...
        def _run_sync_extraction() -> list[dict[str, Any]]:
            if extension in {".json", ".quiz"}:
                with open(staging_file, "rb") as staged:
                    bounded_bytes = staged.read(10 * 1024 * 1024)
                    parsed_assessment = parse_assessment_bank(bounded_bytes, filename)
                return [
                    {
                        "question_text": item.question_text,
                        "question_type": item.question_type,
                        "canonical_type": item.question_type.upper(),
                        "options": item.options,
                        "correct_answer": item.correct_answer,
                        "explanation": item.explanation or "",
                        "topic": item.topic_concept or filename,
                        "points": None,
                        "needs_points_assignment": True,
                        "needs_answer_review": item.correct_answer is None,
                ...
                try:
                    ...
                    return segment_exam_document(parsed_document, distant_keys=answer_keys, filename=filename)
        ...
        if not extracted_candidates and is_plain_text_doc and os.path.exists(staging_file):
            try:
                with open(staging_file, "r", encoding="utf-8", errors="replace") as staged_txt:
                    raw_text = staged_txt.read(1024 * 1024)
                extracted_candidates = segment_exam_document(raw_text, filename=filename)
            except Exception:
                pass
        ...
        for rec in extracted_candidates:
            ...
            points = int(raw_pts) if raw_pts is not None else None
            needs_pts = bool(rec.get("needs_points_assignment", points is None))

            q_type = str(rec.get("canonical_type") or rec.get("question_type") or "MCQ")
            ...
            generated_questions.append(
                GeneratedQuestion(
                    ...
                    options=opts if norm_type == "ESSAY" else (opts if opts else None),
                    correct_answer=confirmed_answer,
                    needs_review=needs_review,
                    needs_answer_review=needs_ans_rev,
                    answer_confidence=answer_conf,
                    explanation=str(rec.get("explanation") or ""),
                    points=points,
                    needs_points_assignment=needs_pts,
                    classification_confidence=rec.get("classification_confidence", "high"),
                    source_page=rec.get("source_page") or rec.get("page_number"),
        ...
        total_points = (
            sum(q.points for q in generated_questions if q.points is not None)
            if any(q.points is not None for q in generated_questions)
            else None
        )
        item_label = "واجب" if target_type == "assignment" else "اختبار"
        return QuizDraftResponse(
            title=f"{item_label} مستخرج: {filename}",
        ...
        if os.path.exists(staging_file):
            try:
                ...
                logger.warning("Failed to remove extraction staging file %s: %s", staging_file, cleanup_err)
...
async def create_manual_quiz(
    ...
    enforce_rate_limit(request, bucket="quiz", limit=10, window_seconds=60)
